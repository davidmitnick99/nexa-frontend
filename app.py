...
if "team_admin_token" not in strl.session_state:
    ...
    if auth_mode == "Sign In To Team Workspace":
        if strl.sidebar.button("🔓 Authenticate Workspace"):
            if not auth_user or not auth_pass:
                strl.sidebar.error("❌ Credentials required.")
            else:
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        strl.success("🤝 Handshake Approved!")
                        strl.rerun()
                    else:
                        strl.sidebar.error("🛑 Invalid team username or password.")
                except Exception as e:
                    strl.sidebar.error(f"💥 Backend transport connection down: {str(e)}")
                    
    elif auth_mode == "Register New Team":
        custom_token = strl.sidebar.text_input("Define Private Admin Modification Token", type="password", help="Use this token to authorize modifications later.")
        
        if strl.sidebar.button("✨ Initialize New Tenant Matrix"):
            if not auth_user or not auth_pass or not custom_token:
                strl.sidebar.error("❌ All fields are required.")
            else:
                try:
                    res = requests.post(f"{API_BASE_URL}/auth/register", json={
                        "team_username": auth_user, 
                        "password": auth_pass, 
                        "admin_token": custom_token.strip()
                    })
                    if res.status_code == 200:
                        strl.sidebar.success("🎉 Registered! Switch radio box above to Sign In.")
                    else:
                        strl.sidebar.error(f"❌ Registration rejected: {res.json().get('detail', 'Unknown error')}")
                except Exception as e:
                    strl.sidebar.error(f"💥 Network transport anomaly: {str(e)}")
else:
...
if not strl.session_state.authenticated:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    with left_col:
                        ...
                        # Dynamic clean string slicing to extract the raw URL even if text leaked inside database
                        if "http" in raw_url:
                            start_idx = raw_url.find("http")
                            clean_url = raw_url[start_idx:].split(" ")[0].replace("'", "").replace('"', '')
                            try:
                                strl.image(clean_url, use_container_width=True)
                            except Exception as img_err:
                                strl.error(f"Could not load image file from extracted URL: {clean_url}")
                        else:
                            strl.info("No valid circuit vector address mapped.")
                    ...
                    with right_col:
                        ...
                        if "budget" in module_data and module_data["budget"]:
                            ...
                            strl.info("No budget data arrays populated yet.")
                        
                        if module_data.get("firmware"):
                            strl.markdown("### 💻 Embedded Architecture Code")
                            strl.code(module_data["firmware"], language="cpp")
                else:
                    strl.info("ℹ️ No module configuration profiles stored under your namespace yet. Pop them into the Secure Admin Portal.")
            except Exception as e:
    ...
    # 2. SECURE ADMIN PORTAL
        ...
        admin_token_input = strl.text_input("Enter Your Registered Admin Modification Token", type="password")
        ...
        if admin_token_input == strl.session_state.team_admin_token and strl.session_state.team_admin_token != "":
            ...
            diag_url = strl.text_input("Circuit Diagram Image URL (Pure Web Link Only)")
            ...
            for i in range(strl.session_state.rows):
                ...
                if it_name.strip():
                    budget_list.append({"item": it_name.strip(), "quantity": int(it_qty), "unit_cost_pkr": int(it_cost)})
            ...
            if strl.button("🚀 Broadcast Module Blueprint to Cloud"):
                ...
                if res.status_code == 200:
                    strl.success("🎉 Successfully synchronized item configurations under your private workspace!")
        elif admin_token_input != "":
            strl.error("🛑 ACCESS VIOLATION: Verification Token mismatch.")
    ...
    # 3. AGENTIC SPRINT PLANNER
        ...
        for k in range(strl.session_state.member_count):
            ...
            m_skills = strl.text_area(f"Technical Skills Matrix & Focus Area", key=f"mem_skills_{k}", value="Firmware systems, C++, microcontroller logic." if k==0 else ("CAD layouts, 3D printing modeling." if k==1 else ("Electronics wiring, inventory mapping." if k==2 else "")))
            
            if m_name.strip():
                team_members_list.append({"name": m_name.strip(), "role": m_role.strip(), "skills": m_skills.strip()})
            strl.markdown("---")
            
        btn_col1, btn_col2, _ = strl.columns([1, 1, 3])
        with btn_col1:
            if strl.button("➕ Add New Member Slot"):
                strl.session_state.member_count += 1
                strl.rerun()
        with btn_col2:
            if strl.button("➖ Remove Member Slot") and strl.session_state.member_count > 1:
                strl.session_state.member_count -= 1
                strl.rerun()
        ...
        if strl.button("🚀 EXECUTE DYNAMIC AGENT STRATEGY GENERATOR"):
            if not team_members_list:
                strl.error("❌ Add team members to proceed.")
            else:
                agent_payload = {
                    "team_id": strl.session_state.team_id,
                ...
                with strl.spinner("Triggering cloud agent sequence..."):
                    try:
                        ...
                        if agent_res.status_code == 200:
                            ...
                            strl.success("🎯 Agent Execution Complete!")
                            strl.markdown("---")
                            ...
                            strl.error(f"❌ Core processing error: {agent_res.text}")
